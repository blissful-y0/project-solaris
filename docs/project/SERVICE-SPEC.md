...
3. 관리자 승인 패널
4. 전투 로비 (게시판) + 전투 세션 (채팅 + AI GM) + **전투 관전 모드** (제3자 실시간 읽기 전용)
5. 일반 RP방 (채팅 + 서사 반영)
6. 대시보드 (내 캐릭터 상태 + 전투 알림 + 도시 뉴스 피드 + 리액션)
7. 캐릭터 도감 (모달 프로필)
8. **진영별 기밀 게시판** (보안국/스태틱 전용 공간, 상대 진영 접근 불가)
9. **캐릭터 관계도** (수동 입력 — 도감 모달 내 "타인과의 관계" 메모 칸)
10. **Solaris Ticker** (대시보드 상단 실시간 시보 — "21:05 보안국 제3구역 검문 강화" 등 자동 생성 텍스트 흘러가는 연출)
11. Discord 알림 봇 (승인/반려 + 전투 + 뉴스)
12. **인프라:** Realtime 채널 설계, 이미지 스토리지(Supabase Storage), OG 메타태그, 커서 기반 페이지네이션, Rate Limiting
