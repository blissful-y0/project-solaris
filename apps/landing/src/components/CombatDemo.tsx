...
function TypingIndicator({ speaker }: { speaker: Speaker }) {
  ...
  return (
    <div className={`flex ${style.align}`}>
      <div
        className="inline-flex gap-1 px-4 py-2 rounded-2xl"
        style={{
          backgroundColor: isSystem ? "rgba(107,114,128,0.15)" : style.bg,
          opacity: 0.7,
        }}
      >
        {[0, 1, 2].map((i) => (
...
function ChatBubble({ message }: { message: ChatMessage }) {
  const style = SPEAKER_STYLES[message.speaker];
      ...
      >
        ...
        {message.label && (
          <span
            ...
            style={{
              color: style.bg,
              textShadow: `0 0 8px ${style.bg}40`,
            }}
          >
            {message.label}
            {message.sublabel && (
        ...
        <div
          ...
          style={{
            backgroundColor: isSystem ? "transparent" : style.bg,
