...
import { useDashboardSession, type DashboardMeCharacter } from "@/components/layout";
...
function mapCharacterToCitizen(character: DashboardMeCharacter): CitizenData {
  return {
...
export default function HomePage() {
  const { me, loading, error, refetch } = useDashboardSession();
  ...
  useEffect(() => {
    ...
    setCitizenData(mapCharacterToCitizen(me.character));
  }, [me]);

  const displayName = useMemo(() => {
    if (citizenData?.status === "approved" && citizenData.name) {
